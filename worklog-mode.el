...
(defun worklog-lang-mode (lang)
  "Return the major-mode symbol for a language/extension string, or nil."
  (let ((l (downcase (or lang ""))))
    (cond
     ((member l '("bash" "sh" "shell" "zsh")) 'sh-mode)
     ((equal l "py") 'python-mode)
     ((member l '("js" "javascript")) 'js2-mode)
     ((equal l "") nil)
     (t (intern (concat l "-mode"))))))
